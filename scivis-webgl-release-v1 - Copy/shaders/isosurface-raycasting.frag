...
vec3 binary_search_iso_surface(vec3 ray_dir, vec3 p1, vec3 p2) {
    vec3 lower_bound = p1;
    vec3 upper_bound = p2;
    vec3 mid_point = upper_bound;
    float v;
    for (int i = 0; i < 10; i++) { // Assuming numIterations is 10
        mid_point = (lower_bound + upper_bound) * 0.5;
        v = sample_data_volume(mid_point);
        if (v > iso_value) {
    ...
    while (inside_volume_bounds(p)) {
        ...
        if ((last_sampled_value < iso_value && current_sampled_value >= iso_value) ||
            ...
            if (binary_search_active) {
                vec3 prev_p = p - ray_dir * sampling_distance;
                p = binary_search_iso_surface(ray_dir, prev_p, p);
            }
